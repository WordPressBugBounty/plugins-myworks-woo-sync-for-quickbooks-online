...
$page_url = 'admin.php?page=myworks-wc-qbo-push&tab=inventory';
...
$MSQS_QL->set_and_get('inventory_push_search');
$inventory_push_search = $MSQS_QL->get_session_val('inventory_push_search');

$total_records = $MSQS_QL->count_woocommerce_product_list($inventory_push_search,true);
...
$wc_inventory_list = $MSQS_QL->get_woocommerce_product_list($inventory_push_search," $offset , $items_per_page",true);
...
<div class="mwqs_page_tab_cont">
	<span class="tab_one active"><a href="<?php echo esc_url($page_url);?>"><?php esc_html_e( 'Products', 'mw_wc_qbo_sync' );?></a></span>
	&nbsp;
	<span class="tab_two"><a href="<?php echo esc_url($page_url);?>&variation=1"><?php esc_html_e( 'Variations', 'mw_wc_qbo_sync' );?></a></span>
</div>

<div class="container">
	<div class="page_title"><h4><?php esc_html_e( 'Inventory Push', 'mw_wc_qbo_sync' );?></h4></div>
						...
						<div class="col s12 m12 l12">
						        ...
						        <div class="panel panel-primary">
									 ...
									 <span class="search_text">Search</span>
									  &nbsp;
									  <input type="text" id="inventory_push_search" value="<?php echo esc_attr($inventory_push_search);?>">
									  &nbsp;		
										...
										<div class="myworks-wc-qbo-sync-table-responsive">
											<table class="table" id="mwqs_inventory_push_table">
												<thead>
													<tr>
														<th width="2%">
														<input type="checkbox" onclick="javascript:mw_qbo_sync_check_all(this,'inventory_push_')">
														</th>
														<th width="5%">ID</th>
														<th width="30%">Product Name</th>
														<th width="14%">SKU</th>
														<th width="8%">Price</th>													
														<th width="7%"><?php echo wp_kses( __( 'WooCommerce<br>Stock', 'mw_wc_qbo_sync' ), array( 'br' => array() ) ); ?></th>
														<th width="9%"><?php echo wp_kses( __( 'QuickBooks<br>Stock', 'mw_wc_qbo_sync' ), array( 'br' => array() ) ); ?></th>
														<th width="7%">Backorders</th>
														<th width="8%"><?php echo wp_kses( __( 'Stock<br>Status', 'mw_wc_qbo_sync' ), array( 'br' => array() ) ); ?></th>
														<th width="6%"><?php echo wp_kses( __( 'Total<br>Sales', 'mw_wc_qbo_sync' ), array( 'br' => array() ) ); ?></th>
												...
												<?php
												$p_val['stock'] = (float) $p_val['stock']; //AL
												if(strpos($p_val['stock'],'.')!==false){
													$stock = number_format(floatval($p_val['stock']),2);
												}else{
													$stock = $p_val['stock'];
												}
												...
												<tr class="wip_vtr" id="tr_wi_<?php echo esc_attr($p_val['ID'])?>">
													<td><input type="checkbox" id="inventory_push_<?php echo esc_attr($p_val['ID'])?>"></td>
													<td><?php echo esc_html($p_val['ID'])?></td>
													<td><a href="<?php echo esc_url(admin_url('post.php?action=edit&post=' . absint($p_val['ID']))) ?>" target="_blank"><?php echo esc_html( $p_val['name'] );?></a></td>
													...
													<td></td>
													<?php endif;?>
													<td><?php echo esc_html($p_val['backorders']);?></td>
...
<?php $sync_window_url = $MSQS_QL->get_sync_window_url();?>
 <script type="text/javascript">
	function search_item(){		
		...
		if(inventory_push_search!=''){			
			window.location = '<?php echo esc_url_raw($page_url);?>&inventory_push_search='+inventory_push_search;
		}else{
	...
	function reset_item(){		
		window.location = '<?php echo esc_url_raw($page_url);?>&inventory_push_search=';
	}
	...
	jQuery(document).ready(function($) {	
		...
		$('#push_selected_inventory_btn').click(function(){
			...
			popUpWindow('<?php echo esc_js($sync_window_url);?>&sync_type=push&item_ids='+item_ids+'&item_type='+item_type,'mw_qs_inventory_push',0,0,650,350);
			return false;
		});
		
		$('#push_all_inventory_btn').click(function(){
			popUpWindow('<?php echo esc_js($sync_window_url);?>&sync_type=push&sync_all=1&item_type='+item_type,'mw_qs_inventory_push',0,0,650,350);
			return false;
		});		
