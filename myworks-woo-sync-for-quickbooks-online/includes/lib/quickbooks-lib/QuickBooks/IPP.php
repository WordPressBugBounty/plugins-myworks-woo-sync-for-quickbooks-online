...
class QuickBooks_IPP
{
	...
	*/
	 ...
	 */
	public function IDS($Context, $realm, $resource, $optype, $xml = '', $ID = null,$uprd=array())
	{
		$IPP = $Context->IPP();
		...
		switch ($IPP->version())
		{
			...
			default:
				return $this->_IDS_v3($Context, $realm, $resource, $optype, $xml, $ID,$uprd);
		}
	}
	...
	protected function _IDS_v3($Context, $realm, $resource, $optype, $xml_or_query, $ID,$uprd=array())
	{
		...
		else if ($optype == QuickBooks_IPP_IDS::OPTYPE_UPLOAD)
		{
			$post = true;
			$url = $this->baseURL() . '/company/' . $realm . '/' . strtolower($optype);
			$xml = $xml_or_query;			
		}
		
		$response = $this->_request($Context, QuickBooks_IPP::REQUEST_IDS, $url, $optype, $xml, $post,$uprd);
	...
	protected function _request($Context, $type, $url, $action, $data, $post = true,$uprd=array())
	{
		...
		if($action == QuickBooks_IPP_IDS::OPTYPE_UPLOAD){
			$boundaryString = $uprd['boundaryString'];
