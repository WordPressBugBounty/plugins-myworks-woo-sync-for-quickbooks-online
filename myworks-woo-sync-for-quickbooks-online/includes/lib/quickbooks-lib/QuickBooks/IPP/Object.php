...
class QuickBooks_IPP_Object
{
	...
	protected function _asXML_v3($indent, $parent, $optype, $flavor)
	{
		...
		$domain_str = '';
		if($indent == 0 && is_array($data) && isset($data['domain']) && ($data['domain'] == 'QBO' || $data['sparse'] == 'QBO')){
		...
		$xml = str_repeat("\t", $indent) . '<' . $this->resource() . ' xmlns="http://schema.intuit.com/finance/v3"'.$domain_str.$sparse_str.'>' . QUICKBOOKS_CRLF;
		
		if(isset($data['domain'])){
			unset($data['domain']);
		}
		
		if(isset($data['sparse'])){
			unset($data['sparse']);
		}
		...
		foreach ($data as $key => $value)
		{
			...
			else
			{
				...
				#New
				$x_prm = '';
				if($this->resource() == 'AttachableRef' && $key == 'EntityRef'){
					$r_ref_a = array(
						'Item',
						'Invoice',
						'SalesReceipt',
					...
					if(isset($data['EntityRef_type']) && !empty($data['EntityRef_type']) &&  in_array($data['EntityRef_type'],$r_ref_a)){
						$x_prm .= ' type="'.$data['EntityRef_type'].'"';
					}
				}
				
				if($this->resource() == 'AttachableRef' && $key == 'EntityRef_type'){
					continue;
				}
				
				$xml .= str_repeat("\t", $indent + 1) . '<' . $key .$x_prm. '>';
				$xml .= QuickBooks_XML::encode($value, $for_qbxml);
				$xml .= '</' . $key . '>' . QUICKBOOKS_CRLF;
			}
		}
